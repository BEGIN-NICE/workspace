...
<body>
	...
	<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
		<ul class="pagination" style="text-align: center; margin-top: 10px;">
			<li <c:if test="${pageBean.currPage==1 }"> class='disabled' </c:if>><a href="${pageContext.request.contextPath }/SearchServlet?method=search&currPage=${pageBean.currPage-1 }&search=${param.search }" aria-label="Previous"><span
			...
			<c:forEach var="i" begin="${pageBean.currPage < 2 ? 1 : pageBean.currPage }"
				...
				<li <c:if test="${pageBean.currPage==i }">class="active"</c:if>>
					<a
					href="${pageContext.request.contextPath }/SearchServlet?method=search&currPage=${i }&search=${param.search }">${i }</a>
				</li>
			</c:forEach>
...
<%-- 			<c:forEach var="i" begin="1"
			...
			<li <c:if test="${pageBean.currPage==pageBean.totalPage }"> class='disabled' </c:if> ><a href="${pageContext.request.contextPath }/SearchServlet?method=search&currPage=${pageBean.currPage+1 }&search=${param.search }" aria-label="Next"> <span aria-hidden="true">&raquo;</span>
			</a></li>
		</ul>
	</div>
	<!-- 分页结束=======================        -->

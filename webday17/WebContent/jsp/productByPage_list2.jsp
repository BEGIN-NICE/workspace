...
<script type="text/javascript">
	...
	function changePageSize() {
		...
		document.getElementById("form1").action = "${pageContext.request.contextPath}/ProductByPageServlet2";
	...
	<form id="form1"
		...
		<table border="1px" width="800">
			...
			<tr>
				...
				每页显示的条数：<input type="text" name = "pageSize" value="${pageBean.pageSize }">&nbsp;&nbsp;
					<input type="button" value = "确定" onclick="changePageSize()">
					<c:if test="${pageBean.currPage > 1 }">
						<a
							href="${pageContext.request.contextPath }/ProductByPageServlet2?currPage=1">首页</a>&nbsp;&nbsp; 
						<a
							href="${pageContext.request.contextPath }/ProductByPageServlet2?currPage=${pageBean.currPage - 1 }">上一页</a>&nbsp;&nbsp; 
					</c:if> 
					<c:forEach var="page" begin="1" end="${pageBean.totalPage }"
						varStatus="status">
						<c:if test="${pageBean.currPage != status.count }">
							<a href="${pageContext.request.contextPath }/ProductByPageServlet2?currPage=${status.count }">${status.count }</a>&nbsp; 
						</c:if>
						<c:if test="${pageBean.currPage == status.count }">
							${status.count } &nbsp;
						</c:if>
				  ...
				  &nbsp;<a
							href="${pageContext.request.contextPath }/ProductByPageServlet2?currPage=${pageBean.currPage + 1 }">下一页</a>&nbsp;&nbsp; 
						<a
							href="${pageContext.request.contextPath }/ProductByPageServlet2?currPage=${pageBean.totalPage}">尾页</a>
					</c:if>
				</td>
			</tr>
		</table>
